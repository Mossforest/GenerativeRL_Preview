...
from grl.generative_models.model_functions.score_script_function import ScoreScriptFunction
...
from grl.numerical_methods.numerical_solvers.sde_script_solver import SDEScriptSolver
...
class DynamicDiffusionModel(nn.Module):
    ...
    Overview:
        Diffusion Model.
    Interfaces:
        ``__init__``, ``sample``, ``score_function``, ``score_matching_loss``.
    """
    ...
    def __init__(self, config: EasyDict) -> None:
        ...
        if hasattr(config, "reverse_path"):
            ...
            # script_path only use the drift item as weight
        self.script_path = GaussianConditionalProbabilityPath(config.script_path)
        ...
        self.script_process = DiffusionProcess(self.script_path)
        self.score_function_ = ScoreScriptFunction(self.model_type, self.diffusion_process, self.script_process)

        if hasattr(config, "solver"):
            self.solver = get_solver(config.solver.type)(**config.solver.args)

    def get_type(self):
        return "DynamicDiffusionModel"

    def sample(
        self,
        script_x: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        condition: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        ...
        Arguments:
            ...
            script_x (:obj:`Union[torch.Tensor, TensorDict, treetensor.torch.Tensor]`): The initial state, if not provided, it will be sampled from the Gaussian distribution.
        ...
        Shapes:
            ...
            script_x: :math:`(N, D)`, where :math:`N` is the batch size of data and :math:`D` is the dimension of the state, which could be a scalar or a tensor such as :math:`(D1, D2)`.
            ...
            x: :math:`(N, D)`, if extra batch size :math:`B` is provided, the shape will be :math:`(B, N, D)`. If script_x is not provided, the shape will be :math:`(B, D)`. If script_x and condition are not provided, the shape will be :math:`(D)`.
        """
    ...
    def sample_forward_process(
        ...
        script_x: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        condition: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        classifier_weight: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        ...
        assert (
            script_x.shape[0] == condition.shape[0]
        ), "The batch size of script_x and condition must be the same"
        data_batch_size = script_x.shape[0]
        ...
        if isinstance(solver, SDEScriptSolver):
            assert (
            ...
            sde = self.diffusion_process.reverse_sde(
                ...
                reverse_script_function=self.script_path.diffusion,
            )
            if with_grad:
        ...
        if isinstance(data, torch.Tensor):
            ...
            if batch_size is None:
                if script_x is None and condition is None:
    ...
    def score_matching_loss(
        ...
        # classifier_weight = self.classifier(script_x, condition['action'], condition['background'])
        # classifier_loss = self.classifier.loss(classifier_weight, script_x, x)
        score_loss = self.score_function_.score_matching_loss(
            self.model, x, script_x, condition, classifier_weight, self.gaussian_generator, average
        )
        return score_loss

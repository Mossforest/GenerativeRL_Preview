...
from grl.generative_models.model_functions.score_draft_function import ScoreDraftFunction
...
from grl.numerical_methods.numerical_solvers.sde_draft_solver import SDEDraftSolver
...
class DynamicDiffusionModel(nn.Module):
    ...
    def sample(
        self,
        draft_x: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        condition: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor],
        classifier_weight: Union[torch.Tensor, TensorDict, treetensor.torch.Tensor] = None,
        ...
        Arguments:
            ...
            draft_x (:obj:`Union[torch.Tensor, TensorDict, treetensor.torch.Tensor]`): The initial state, if not provided, it will be sampled from the Gaussian distribution.
        ...
        Shapes:
            ...
            draft_x: :math:`(N, D)`, where :math:`N` is the batch size of data and :math:`D` is the dimension of the state, which could be a scalar or a tensor such as :math:`(D1, D2)`.
            ...
            x: :math:`(N, D)`, if extra batch size :math:`B` is provided, the shape will be :math:`(B, N, D)`. If draft_x is not provided, the shape will be :math:`(B, D)`. If draft_x and condition are not provided, the shape will be :math:`(D)`.
        """
    ...
    def sample_forward_process(
        ...
        if isinstance(solver, SDEDraftSolver):
            ...
            sde = self.diffusion_process.reverse_sde(
                ...
                draft_x=draft_x,
                condition=condition,
                reverse_diffusion_function=self.reverse_diffusion_process.diffusion,  # TODO: if current diffusion & drift \nogood, then could consider diffusion_draftver
                reverse_diffusion_squared_function=self.reverse_diffusion_process.diffusion_squared,
                reverse_draft_function=self.reverse_diffusion_process.draft,
            )
            if with_grad:
        ...
        if isinstance(data, torch.Tensor):
            ...
            if batch_size is None:
                if draft_x is None and condition is None:

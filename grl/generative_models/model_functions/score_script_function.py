...
class ScoreScriptFunction:
    """
    Overview:
        Model of Score function in Score-based script generative model.
    Interfaces:
    ...
    def __init__(
        ...
        self.script_process = script_process
        assert self.model_type in [
            "score_script_function",
    ...
    def forward(
        ...
        if self.model_type == "score_script_function":
    ...
    def score_matching_loss(
        ...
        if self.model_type == "score_function":
            ...
            # TODO: make sure l(t) * weight size correct: should be [B(t), *S(weight)]
            l_tilde = self.script_process.scale(t_random) * classifier_weight
            x_t = self.process.scale(t_random, x) * (l_tilde * x + (1 - l_tilde) * script_x) + std * noise

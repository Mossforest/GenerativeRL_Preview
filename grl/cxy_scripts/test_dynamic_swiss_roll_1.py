...
def save_checkpoint(
    diffusion_model, diffusion_model_iteration, path
):
    if not os.path.exists(path):
        os.makedirs(path)
    torch.save(
        ...
        f=os.path.join(
            path, f"checkpoint_{diffusion_model_iteration}.pt"
        ),
    )
...
def main(wandb_run):
    ...
    dataset = DynamicSwissRollDataset(config.dataset, train=True)
    data_loader = torch.utils.data.DataLoader(
        dataset, batch_size=config.parameter.batch_size, shuffle=True
    )
    data_generator = get_train_data(data_loader)
    
    test_dataset = DynamicSwissRollDataset(config.dataset, train=False)
    ...
    for key, val in eval_data.items():
        eval_data[key] = val.to(device)
    
    
    # ! build model !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    diffusion_model = DynamicDiffusionModel(config=config.model.diffusion_model).to(config.model.diffusion_model.device)
    ...
    for train_iter in track(
        ...
        for key, val in train_data.items():
            train_data[key] = val.to(device)
        train_x, train_a, train_bg, train_next_x = train_data['state'], train_data['action'], train_data['background'], train_data['next_state']
        train_condition = {'action': torch.zeros_like(train_a), 'dynamic': train_bg}
        ...
        diffusion_training_loss = diffusion_loss
        ...
        if train_iter % 100 == 0:
            log.info(
                f"iteration {train_iter}, diffusion model loss {diffusion_loss.item():6f}, training loss {diffusion_training_loss.item():6f}, moving average loss {moving_average_loss:6f}"
            )
        ...
        if (
            ...
            with torch.no_grad():
                ...
                eval_condition = {'action': torch.zeros_like(eval_a), 'dynamic': eval_bg}

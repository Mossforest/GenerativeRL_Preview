...
sub_name = "medium-mlpdecoder-Unet"
exp_name = f"{proj_name}-{sub_name}"

env_id='hopper-medium-v2'
x_size = 11
condition_size=3  # include action and background
...
if __name__ == "__main__":
    ...
    for iteration in track(range(config.parameter.num_iterations), description="Training"):
        ...
        for index, batch_data in enumerate(replay_buffer):
            ...
            state=batch_data["s"].to(config.device)
            next_state=batch_data["s_"].to(config.device)

            flow_model.train()
            loss = flow_model.flow_matching_loss(x0=state, x1=next_state, condition=action)
            optimizer.zero_grad()
            loss.backward()

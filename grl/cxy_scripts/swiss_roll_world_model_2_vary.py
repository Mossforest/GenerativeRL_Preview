...
config = EasyDict(
    dict(
        ...
        parameter=dict(
            ...
            checkpoint_path="./checkpoint-swiss-roll-dynamic-icfm-world-model",
            video_save_path="./video-swiss-roll-dynamic-icfm-world-model",
            device=device,
        ),
    )
)
...
if __name__ == "__main__":
    ...
    customized_dataset.plot(customized_dataset.data['state'], customized_dataset.data['next_state'], customized_dataset.data['action'], tag='dataset_plot')

    x0, x1, action = customized_dataset.data['state'], customized_dataset.data['next_state'], customized_dataset.data['action']
    #
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        # if True:
            ...
            condition_eval = torch.cat((action_eval, background_eval), dim=1)
            condition_eval = torch.tensor(condition_eval).to(config.device)
        ...
        if config.parameter.training_loss_type == "flow_matching":
            batch_condition = torch.cat((batch_data['action'], batch_data['background']), dim=1)
            loss = flow_model.flow_matching_loss(x0=batch_data['next_state'], x1=batch_data['state'], condition=batch_condition)
        else:

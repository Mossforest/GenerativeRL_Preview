    ...
    config = EasyDict(
        dict(
            ...
            parameter=dict(
                ...
                eval_freq=50,
                checkpoint_freq=50,
                checkpoint_path="/root/world_model_atari/atari_world_model/checkpoint-atari-world-model",
                video_save_path="/root/world_model_atari/atari_world_model/video-atari-world-model",
                device=device,
            ),
        )
    )
...
def pipeline(config):
    ...
    for iteration in range(config.parameter.iterations):
        ...
        if iteration > 0 and iteration % config.parameter.eval_freq == 0:
            ...
            x_t = (
                diffusion_model.sample_forward_process(t_span=t_span, x_0=x_0, condition=condition)
                .cpu()
                .detach()
            )
        ...
        for obs, reward, action, next_obs, done in track(data_loader, description=f"Epoch {iteration}"):
            ...
            if config.parameter.training_loss_type == "flow_matching":
                loss = diffusion_model.flow_matching_loss(x=next_obs, condition=condition)
            elif config.parameter.training_loss_type == "score_matching":
                loss = diffusion_model.score_matching_loss(x=next_obs, condition=condition)
            else:
